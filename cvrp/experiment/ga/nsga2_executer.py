"""NSGA-II experiment orchestrator for the Multi-Objective CVRP.
...
and run the NSGA2 wrapper, persisting results to CSV.
"""
...
from cvrp.ga.nsga2 import NSGA2
...
class NSGA2Executer:
    """Runs NSGA-II experiments on the CVRP benchmark instances.

    Builds a CVRPBenchmark from each .vrp file, instantiates the NSGA2
    wrapper with the variators matching the chosen representation, and
    ...
    def run_single_experiment(
        ...
        **kwargs: Any,
        ...
        """Run NSGA-II once on a single instance.

        Args:
            instance_name: Filename inside instances_folder, e.g. "X-n101-k25.vrp".
            seed: Random seed forwarded to NSGA2.run().
            **kwargs: Hyperparameters forwarded to NSGA2.__init__()
                (pop_size, max_generations, crossover_rate, etc.).

        Returns:
            The NSGA2 wrapper after run() completes. Access
            .final_archive, .history, .num_evaluations, .num_generations.
        """
        benchmark = self._build_benchmark(instance_name)
        nsga2 = NSGA2(benchmark=benchmark, variators=self.variators, **kwargs)
        nsga2.run(seed=seed)
        return nsga2
    ...
    def run_all_experiments(
        ...
        **kwargs: Any,
        ...
        Args:
            experiment_folder: Output folder for CSVs (e.g. "experiments/nsga2/").
            n_repeat: Number of independent runs per instance.
            overwrite: If True, wipe the output folder before writing.
            **kwargs: Hyperparameters forwarded to NSGA2 (applied to all
